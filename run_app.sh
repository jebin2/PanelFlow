...
# `-m panelflow.batch` needs the repo on the path, and the inbox is resolved
...
pkill -f "PanelFlow_env/.*python.*panelflow\.batch" 2>/dev/null || true
sleep 1
...
exec taskset -c "$CORE_LIST" nice -n 15 "$PYTHON" -m panelflow.batch "$@"
