...
import gemini_history_processor
import resize_with_aspect
import remove_sound_effect
...
Confirm focus on core narrative only"""
	...
	def get_source_path(self, path=None):
		...
		for path in dirs:
			if path not in added_dir and (not self.main_instance.source_name or (self.main_instance.source_name in path)):
				return path
